                ...
                <?php if (!empty($reviews)): ?>
                    <?php foreach ($reviews as $review): ?>
                        <div class="mb-3 border-bottom pb-3">
                            ...
                            <small class="text-muted">โพสต์เมื่อ: <?= htmlspecialchars($review['created_at']) ?></small>
                            <form action="delete_review.php" method="POST" class="d-inline">
                                <input type="hidden" name="review_id" value="<?= $review['review_id'] ?>">
                                <button type="submit" class="btn btn-danger btn-sm mt-2" onclick="return confirm('คุณต้องการลบรีวิวนี้หรือไม่?')">ลบ</button>
                            </form>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
